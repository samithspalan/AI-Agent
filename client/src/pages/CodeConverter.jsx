...
const CodeConverter = () => {
  ...
  const handleConvert = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/convert-code', {
        method: 'POST',
        headers: {
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-8">
                ...
                <div className="flex items-center gap-3 backdrop-blur-md p-4 rounded-3xl border border-white/5 bg-white/5">
                    ...
                    <select
                        ...
                        className={`px-5 py-2.5 rounded-xl border border-white/10 outline-none cursor-pointer font-bold text-xs uppercase tracking-widest transition-all ${
                            isDark ? 'bg-[#0A1120] text-indigo-300 hover:border-indigo-500/40' : 'bg-white text-indigo-600 shadow-sm'
                        }`}
                    >
                        {languages.map((lang) => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto_1fr] gap-6 items-center">
            ...
            <div className="flex flex-col items-center gap-6">
                <button
                   ...
                   className={`relative w-16 h-16 rounded-[2rem] flex items-center justify-center transition-all duration-500 group ${
                       inputCode.trim() 
            ...
            {/* Right Column: Result */}
            <div className={`relative flex flex-col h-[650px] border rounded-3xl overflow-hidden backdrop-blur-md transition-all shadow-2xl ${
                outputCode ? 'border-indigo-500/30 shadow-indigo-500/5' : cardBg
            }`}>
                <div className={`flex items-center justify-between px-6 py-4 border-b ${isDark ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
                    ...
                    <button
                        ...
                        className={`transition-colors p-1.5 rounded-lg ${
                            isCopied ? 'text-emerald-400 bg-emerald-400/10' : 'text-slate-400 hover:text-white hover:bg-indigo-500/10'
                        }`}
                    >
                ...
                <div className="relative flex-1 overflow-hidden">
                    ...
                    <textarea
                      ...
                      className={`w-full h-full p-8 code-textarea outline-none resize-none bg-transparent text-sm leading-relaxed ${
                         outputCode ? (isDark ? 'text-indigo-300' : 'text-indigo-800') : (isDark ? 'text-slate-700' : 'text-slate-400')
                      }`}
                      style={{ fontFamily: "'JetBrains Mono', monospace" }}
                    />
                </div>
